...
def Sign(ecc,G,n,M,dA,Z):
    ...
    kG=SM2.ecc_multiply(ecc,hex(k)[2:],G)   #坐标点,列表十六进制形式  椭圆曲线上的运算全为字符串,数据类型需要转化
...
def get_pk_from_sign(ecc,n,M,sign,Z):
    ...
    kGy=hex(kGy)[2:]
    kG=[kGx,kGy]
    sG=SM2.ecc_multiply(ecc,s,G)
    #print("sG",sG)
    temp1=findModInverse(int(s,16)+r,n1)
    #print("temp1",temp1)
    temp2=SM2.ecc_diff_sub(ecc,kG,sG)
    #print("temp2", temp2)
    PA=SM2.ecc_multiply(ecc,hex(temp1)[2:],temp2)
    return PA
...
G=[Gx,Gy]   #基点
tx="736475"  #sdu 编码
pA=SM2.ecc_multiply(ecc,dA,G)
print("公钥:",pA)
sign=Sign(ecc, G,n, tx, dA, Z)
start=time.time()
PA=get_pk_from_sign(ecc,n,tx,sign,Z)
print("签名得到的公钥:",PA)
end=time.time()
print("Deduce public key from signature 消耗时间:",end-start,"s")
